...
void RobotContainer::ConfigureButtonBindings() {
  ...
  //Coral
  frc2::JoystickButton(&m_copilotController, 6).WhileTrue(SetCoralPosition(&m_coral,16.6904).ToPtr()); // intake
  frc2::JoystickButton(&m_copilotController, 8).WhileTrue(SetCoralPosition(&m_coral,5).ToPtr()); //home
  // leftTriggerPressed.WhileTrue(SetCoralPosition(&m_coral,10).ToPtr()); // REMEMBER CHANGE ANGLE Shoot pos
  frc2::JoystickButton(&m_copilotController, 3).WhileTrue(SetCoralPosition(&m_coral, 24.833).ToPtr());//L4 pos
  rightTriggerPressed.WhileTrue(SetCoralPosition(&m_coral, 23.928).ToPtr());//shoot
